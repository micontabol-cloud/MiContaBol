...
export default function Clientes() {
  ...
  async function cargar() {
    ...
    const [{ data: cl, error: errCl }, { data: v }, { data: c }] = await Promise.all([
      ...
      supabase.from('vista_resumen_clientes').select('*').eq('empresa_id', empresaId),
    ...
    setClientes(cl || [])
    setResumen(new Map((v || []).map((r) => [r.cliente_id, r])))
    setCxc(c || [])
    setCargando(false)
  }
  ...
  function totalCompradoDe(cliente) {
    return Number(resumen.get(cliente.id)?.total_comprado || 0)
  }

  function saldoPendienteDe(cliente) {
    return cxc
  ...
  const conMetricas = clientes
    .filter((c) => c.activo)
    .map((c) => ({ ...c, total: totalCompradoDe(c), pendiente: saldoPendienteDe(c) }))
            ...
            {clientesVisibles.map((c) => {
              ...
              return (
                <tr key={c.id} style={{ borderBottom: '1px solid #E6ECF3', opacity: c.activo ? 1 : 0.5 }}>
                  ...
                  <td style={{ padding: '4px 8px', textAlign: 'right' }}>{totalCompradoDe(c).toFixed(2)}</td>
                  <td
                    style={{
                      padding: '4px 8px',
                      textAlign: 'right',
