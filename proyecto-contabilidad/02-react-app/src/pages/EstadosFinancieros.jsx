  ...
  return (
    <main style={{ maxWidth: 700, margin: '3rem auto', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '1rem', flexWrap: 'wrap' }}>
        <div>
          <h1 style={{ margin: 0 }}>Estados financieros</h1>
          <p style={{ color: '#64748B', margin: '0.25rem 0 0' }}>Solo incluye asientos confirmados.</p>
        </div>
        <BotonesExportar
          titulo="Estados financieros"
          empresa={nombreEmpresa}
          subtitulo={desde || hasta ? `Período: ${desde || 'inicio'} al ${hasta || 'hoy'}` : 'Acumulado desde el inicio'}
          deshabilitado={filas.length === 0}
          secciones={[
            {
              titulo: 'ESTADO DE RESULTADOS — Ingresos',
              columnas: ['Cuenta', 'Monto'],
              alineacionDerecha: [1],
              filas: ingresos.map((f) => [`${f.codigo} ${f.nombre}`, f.saldo.toFixed(2)]),
            ...
            },
            {
              ...
              columnas: ['Cuenta', 'Monto'],
              alineacionDerecha: [1],
              filas: gastos.map((f) => [`${f.codigo} ${f.nombre}`, f.saldo.toFixed(2)]),
              total: ['Total gastos', totalGastos.toFixed(2)],
            },
            {
              titulo: 'Resultado',
              filas: [[utilidadNeta >= 0 ? 'Utilidad neta' : 'Pérdida neta', Math.abs(utilidadNeta).toFixed(2)]],
              alineacionDerecha: [1],
            },
            {
              titulo: 'BALANCE GENERAL — Activo',
              columnas: ['Cuenta', 'Monto'],
              alineacionDerecha: [1],
              filas: activos.map((f) => [`${f.codigo} ${f.nombre}`, f.saldo.toFixed(2)]),
              total: ['Total activo', totalActivo.toFixed(2)],
            },
            {
              titulo: 'Pasivo',
              columnas: ['Cuenta', 'Monto'],
              alineacionDerecha: [1],
              filas: pasivos.map((f) => [`${f.codigo} ${f.nombre}`, f.saldo.toFixed(2)]),
              total: ['Total pasivo', totalPasivo.toFixed(2)],
            },
            {
              titulo: 'Patrimonio',
              columnas: ['Cuenta', 'Monto'],
              alineacionDerecha: [1],
              filas: [
                ...patrimonios.map((f) => [`${f.codigo} ${f.nombre}`, f.saldo.toFixed(2)]),
                ['Utilidad del ejercicio', utilidadNeta.toFixed(2)],
              ],
              total: ['Total pasivo + patrimonio', totalPasivoMasPatrimonio.toFixed(2)],
            },
          ]}
        />
      </div>
