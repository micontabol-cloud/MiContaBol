 ...
 * Registra una visita.
 *
 * El .then() no es decorativo: en Supabase la petición no se envía al
 * crearla, sino cuando alguien espera su resultado. Sin esto, la
 * llamada se escribe pero nunca sale del navegador — que es
 * exactamente por qué el contador quedaba en cero.
 *
 * No se usa await para no demorar lo que el usuario está haciendo.
 */
function registrarVisita(slug, productoId = null) {
  supabase
    .rpc('registrar_visita_catalogo', { p_slug: slug, p_producto_id: productoId })
    .then(
      () => {},
      () => {} // si falla, se pierde la visita pero nada más
    )
}
...
export default function CatalogoPublico() {
  ...
  function alTocarProducto(item) {
    registrarVisita(slug, item.producto_id)
