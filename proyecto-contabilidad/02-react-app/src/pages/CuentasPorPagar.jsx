...
export default function CuentasPorPagar() {
  ...
  async function cargar() {
    ...
    const [{ data: cxc, error: errCxc }, { data: cts }] = await Promise.all([
      supabase.from('vista_cuentas_por_pagar').select('*').eq('empresa_id', empresaId).order('fecha'),
      supabase
        .from('plan_cuentas')
  ...
  return (
    <main style={{ maxWidth: 800, margin: '3rem auto', fontFamily: 'sans-serif' }}>
      ...
      {cargando ? (
        ...
        <p>No le debes nada a ningún proveedor. 🎉</p>
      ) : (
        <>
          ...
          <p style={{ fontWeight: 'bold', marginTop: '1rem' }}>Total por pagar: {totalPendiente.toFixed(2)}</p>
        </>
      )}
    </main>
  )
