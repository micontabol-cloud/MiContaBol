...
export default function NuevoComprobante() {
  ...
  const preview = (() => {
    ...
    const cuentaPrincipal = cuentaPorId.get(cuentaPrincipalId)
    ...
    const faltantes = []
    if (!cuentaPrincipal) faltantes.push(tipo === 'venta' ? 'la cuenta de ingreso' : 'la cuenta de gasto')
    if (!cuentaContra) {
      faltantes.push(
        esCredito
          ? tipo === 'venta'
        ...
        ? [
            {
              icono: esCredito ? '🤝' : '💰',
              frase: esCredito
                ? `${clienteProveedor || 'El cliente'} te queda debiendo Bs ${montoNum.toFixed(2)}.`
                : `Entran Bs ${montoNum.toFixed(2)} a ${cuentaContra?.nombre || metodo.nombre}.`,
              cuenta: cuentaContra,
              debe: montoNum,
              haber: 0,
            },
            {
              icono: '📈',
              frase: `Registras Bs ${montoNum.toFixed(2)} como ingreso en "${cuentaPrincipal?.nombre || 'tu cuenta de ingreso'}".`,
