...
// igual, no queda rota.
const rubros = [
  { icon: '👠', label: 'Zapaterías', texto: 'Controla tu inventario por modelo y talla.', foto: '/rubros/zapateria.jpg' },
  { icon: '💍', label: 'Joyerías', texto: 'Cada pieza con su costo y su margen real.', foto: '/rubros/joyeria.jpg' },
  { icon: '👕', label: 'Boutiques', texto: 'Tallas y colores, todo en un solo producto.', foto: '/rubros/boutique.jpg' },
  { icon: '🛒', label: 'Minimarkets', texto: 'Avisos de vencimiento antes de perder plata.', foto: '/rubros/minimarket.jpg' },
  { icon: '🔧', label: 'Ferreterías', texto: 'Cientos de artículos, encontrados en segundos.', foto: '/rubros/ferreteria.jpg' },
  { icon: '💊', label: 'Farmacias', texto: 'Control por lote y fecha de caducidad.', foto: '/rubros/farmacia.jpg' },
]
...
export default function Landing() {
  ...
  return (
    ...
    >
      <header className="landing-header">
        <div className="landing-container landing-header-inner">
          ...
          <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
            <a href="#precios" style={{ fontSize: '0.92rem' }}>
              Precios
            </a>
            <Link to="/login">
          ...
          <div className="landing-grid-3" style={{ marginTop: '2rem' }}>
            {rubros.map((r) => (
              <div key={r.label} className="rubro-card">
                <div
                  className={'rubro-foto' + (r.foto ? ' con-foto' : '')}
                  style={r.foto ? { backgroundImage: `url(${r.foto})` } : undefined}
                >
                  {r.foto ? <span className="rubro-chip">{r.icon}</span> : r.icon}
                </div>
      ...
      {/* PRECIOS */}
      <section className="landing-section bg-gris" id="precios">
        <div className="landing-container">
          <h2 className="landing-h2" style={{ textAlign: 'center' }}>Un precio claro, sin sorpresas</h2>
          <p style={{ textAlign: 'center', color: '#64748B', marginTop: '0.5rem' }}>
            Empieza con <strong>un mes gratis</strong> del plan Negocio. Sin tarjeta, sin compromiso.
          </p>

          <div style={{ display: 'flex', justifyContent: 'center', marginTop: '1.5rem' }}>
            <div style={{ display: 'flex', gap: '0.4rem', background: '#FFFFFF', padding: '0.25rem', borderRadius: 999, border: '1px solid var(--color-border)' }}>
              {[
          ...
          {planes.length === 0 ? (
            <p style={{ textAlign: 'center', color: '#A3AFBF', marginTop: '2rem' }}>Cargando planes...</p>
          ) : (
              ...
              {planes.map((p) => {
                ...
                return (
                  ...
                  >
                    ...
                    <h3 style={{ margin: recomendado ? '0.5rem 0 0.25rem' : '0 0 0.25rem', fontSize: '1.15rem', color: 'var(--color-navy)' }}>
                      {p.nombre}
                    </h3>
                    <p style={{ color: '#64748B', fontSize: '0.88rem', margin: '0 0 1rem', minHeight: 42 }}>
                      {p.descripcion}
                    </p>

                    <p style={{ margin: 0 }}>
                      <span style={{ fontSize: '2rem', fontWeight: 800, color: 'var(--color-navy)' }}>
                        Bs {Number(precio).toFixed(0)}
                      </span>
                      <span style={{ color: '#64748B', fontSize: '0.9rem' }}>
                        {ciclo === 'anual' ? ' /año' : ' /trimestre'}
                      </span>
                    </p>
                    <p style={{ margin: '0.15rem 0 1.1rem', fontSize: '0.85rem', color: '#22C55E', fontWeight: 600 }}>
                      Equivale a Bs {(precio / meses).toFixed(0)} al mes
                    </p>
                    ...
                    <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                      ...
                      {[
                        ...
                        >
                          <span style={{ color: f.ok ? '#22C55E' : '#E6ECF3', flexShrink: 0, marginTop: 2 }}>
          ...
          <p style={{ textAlign: 'center', color: '#64748B', fontSize: '0.9rem', marginTop: '1.75rem' }}>
            Pagas por QR o transferencia, desde tu propio banco. Sin cobros automáticos: tú decides cuándo renovar.
          </p>
        </div>
      </section>
