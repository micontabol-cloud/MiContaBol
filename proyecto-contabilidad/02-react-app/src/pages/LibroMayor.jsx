  ...
  return (
    <main style={{ maxWidth: 800, margin: '3rem auto', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '1rem', flexWrap: 'wrap' }}>
        ...
        <BotonesExportar
          titulo={`Libro mayor — ${cuentaActual ? cuentaActual.nombre : ''}`}
          empresa={nombreEmpresa}
          subtitulo={desde || hasta ? `Período: ${desde || 'inicio'} al ${hasta || 'hoy'}` : 'Acumulado desde el inicio'}
          deshabilitado={filasConSaldo.length === 0}
          secciones={[
            {
              columnas: ['Fecha', 'N°', 'Glosa', 'Debe', 'Haber', 'Saldo'],
              alineacionDerecha: [3, 4, 5],
              filas: [
                ...(desde ? [['', '', `Saldo inicial (antes de ${desde})`, '', '', saldoInicial.toFixed(2)]] : []),
                ...filasConSaldo.map((m) => [
                  m.fecha,
                  String(m.numero),
                  m.asiento_glosa || '',
                  Number(m.debe).toFixed(2),
                  Number(m.haber).toFixed(2),
                  m.saldo.toFixed(2),
                ]),
              ],
            },
