...
export default function Productos() {
  ...
  async function cargar() {
    ...
    const [prodRes, catRes, ctasRes, empRes, varRes] = await Promise.all([
      supabase.from('productos').select('*').eq('empresa_id', empresaId).is('eliminado_at', null).order('codigo'),
    ...
    supabase.rpc('uso_inventario', { p_empresa_id: empresaId }).then(({ data }) => setUso(data))
    if (empRes.data) {
  ...
  // En un formulario HTML, Enter en cualquier campo lo envía. Mucha
  // gente usa Enter para pasar al siguiente campo, y así terminaba
  // creando productos a medias sin querer. Aquí Enter avanza al
  // siguiente campo, y el producto solo se crea con el botón.
  function manejarEnter(e) {
    if (e.key !== 'Enter') return
    e.preventDefault()

    const campos = Array.from(e.currentTarget.querySelectorAll('input, select')).filter(
      (el) => !el.disabled && el.type !== 'checkbox'
    )
    ...
    <main style={{ maxWidth: 1000, fontFamily: 'sans-serif' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '1rem', flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
          ...
          <Link to={`/empresas/${empresaId}/inventario/importar`}>
            <button type="button">Importar desde Excel</button>
          </Link>
          <Link to={`/empresas/${empresaId}/inventario/papelera`}>
      ...
      {/* Contador del plan: se muestra desde el principio, no solo al
          chocar con el tope — así el límite no llega de sorpresa. */}
      {uso?.limite && (
        <div
          style={{
            display: 'flex',
            ...
            gap: '0.9rem',
            flexWrap: 'wrap',
            background: uso.usados >= uso.limite ? 'rgba(245, 158, 11, 0.1)' : '#F7F9FC',
            border: `1px solid ${uso.usados >= uso.limite ? 'rgba(245, 158, 11, 0.4)' : '#E6ECF3'}`,
            borderRadius: 12,
            padding: '0.7rem 1rem',
        ...
        >
          <span style={{ flex: 1, minWidth: 200 }}>
            Usas <strong>{uso.usados}</strong> de <strong>{uso.limite}</strong> productos del plan {uso.plan}.
            <span style={{ color: '#A3AFBF' }}> Cada talla y color cuenta como uno.</span>
          </span>
          <div style={{ flex: 1, minWidth: 140, height: 6, background: '#E6ECF3', borderRadius: 999, overflow: 'hidden' }}>
            <div
              style={{
                width: `${Math.min(100, (uso.usados / uso.limite) * 100)}%`,
                height: '100%',
                background: uso.usados >= uso.limite ? '#F59E0B' : '#22C55E',
              }}
            />
          </div>
          <Link to="/suscripcion" style={{ fontSize: '0.85rem' }}>
            Ver planes &rarr;
          </Link>
        </div>
      )}
      ...
      <details style={{ margin: '1rem 0', border: '1px solid #E6ECF3', borderRadius: 12, padding: '0.75rem' }}>
        ...
        <form
          ...
          style={{ display: 'flex', gap: '0.5rem', alignItems: 'flex-end', flexWrap: 'wrap', marginTop: '0.75rem' }}
        >
          <label>
            Nombre
            <br />
      ...
      <form
        ...
        onKeyDown={manejarEnter}
        style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', alignItems: 'flex-end', marginBottom: '1.5rem' }}
      >
        <label>
          Código
          <br />
        ...
        <button className="btn-hero" type="submit" disabled={guardando}>
          Agregar producto
        </button>

        {(codigo.trim() || nombre.trim()) && !parseFloat(precioVenta) && (
          <p style={{ width: '100%', margin: '0.4rem 0 0', fontSize: '0.85rem', color: '#F59E0B' }}>
            Le falta el precio de venta. Puedes agregarlo igual y completarlo después.
          </p>
        )}
      </form>
