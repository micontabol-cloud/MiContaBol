...
export default function Catalogos() {
  ...
  async function cargar() {
    ...
    const [catRes, empRes] = await Promise.all([
      supabase
        .from('catalogos')
        .select('*, catalogo_items(id), catalogo_visitas(id)')
  ...
  // Solo contamos las aperturas del catálogo, no los toques a
  // productos, para que el número diga "cuánta gente lo vio".
  const visitasDe = (c) => (c.catalogo_visitas || []).length
  const totalVisitas = catalogos.reduce((s, c) => s + visitasDe(c), 0)
  const masVisto = [...catalogos].sort((a, b) => visitasDe(b) - visitasDe(a))[0]
  ...
  if (totalVisitas > 0) {
    hallazgos.push({
      ...
      texto: (
        <>
          Tus catálogos suman <strong>{totalVisitas}</strong> {totalVisitas === 1 ? 'visita' : 'visitas'}
          {masVisto && visitasDe(masVisto) > 0 && (
            <>
              , y el que más miran es <strong>{masVisto.nombre}</strong>
            </>
          )}
          .
        </>
  ...
  return (
    <main style={{ maxWidth: 900, fontFamily: 'sans-serif' }}>
      ...
      <p>
        <Link to={`/empresas/${empresaId}/catalogos/identidad`}>
          Configurar la imagen de tu negocio (logo, color y contacto) &rarr;
        </Link>
      </p>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ textAlign: 'left', borderBottom: '1px solid #E6ECF3' }}>
              ...
              <th style={{ padding: '4px 8px', textAlign: 'right' }}>Visitas</th>
              ...
              <tr key={c.id} style={{ borderBottom: '1px solid #E6ECF3' }}>
                ...
                <td
                  style={{
                    padding: '6px 8px',
                    textAlign: 'right',
                    fontWeight: visitasDe(c) > 0 ? 600 : 400,
                    color: visitasDe(c) > 0 ? '#1F3A5F' : '#A3AFBF',
                  }}
                >
                  {visitasDe(c) || '—'}
                </td>
